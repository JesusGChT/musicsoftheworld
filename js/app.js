...
// Para instrumentos usamos la API de Wikimedia Commons directamente
// buscando el archivo de imagen más adecuado por título exacto
async function loadCommonsImage(filename, size = 400) {
  try {
    const url = `https://commons.wikimedia.org/w/api.php?action=query&titles=File:${encodeURIComponent(filename)}&prop=imageinfo&iiprop=url&iiurlwidth=${size}&format=json&origin=*`;
    const d = await (await fetch(url)).json();
    const page = Object.values(d.query.pages)[0];
...
function buildInstruments(region) {
  ...
  region.instruments.forEach(inst => {
    ...
    // Cargar imagen del instrumento desde Wikimedia Commons (archivo exacto)
    loadCommonsImage(inst.commons_image, 120).then(url => {
      const wrap = document.getElementById(thumbId);
      if (wrap && url) {
        wrap.innerHTML = `<img src="${url}" alt="${inst.name}"/>`;
      } else if (wrap) {
        // Fallback: Wikipedia artículo
        loadWikiImage(inst.wikipedia, 'es', 200).then(u => {
          if (wrap && u) wrap.innerHTML = `<img src="${u}" alt="${inst.name}"/>`;
          else if (wrap) wrap.textContent = '🎵';
        });
      }
    });
  });
...
window.openInstrumentModal = async function(inst) {
  ...
  imgWrap.innerHTML = `<div style="width:100%;height:260px;background:var(--sand);display:flex;align-items:center;justify-content:center;color:#B0A898;font-size:var(--fs-sm)">Cargando imagen…</div>`;

  // Intentar primero Commons con archivo exacto, luego Wikipedia
  loadCommonsImage(inst.commons_image, 600).then(url => {
    if (url) {
      ...
      loadWikiImage(inst.wikipedia, 'es', 600).then(u => {
        if (u) {
          imgWrap.innerHTML = `<img class="modal-img" src="${u}" alt="${inst.name}"/><div class="modal-img-cap">© Wikipedia · Wikimedia Commons CC BY-SA</div>`;
        } else {
  ...
  document.getElementById('modalAudio').innerHTML = `
    ...
    <a class="yt-btn" href="https://www.youtube.com/results?search_query=${encodeURIComponent(inst.name + ' instrumento solo tradicional')}" target="_blank">
      <svg class="yt-icon" viewBox="0 0 24 24" fill="currentColor"><path d="M23.5 6.19a3.02 3.02 0 00-2.12-2.14C19.53 3.5 12 3.5 12 3.5s-7.53 0-9.38.55A3.02 3.02 0 00.5 6.19C0 8.05 0 12 0 12s0 3.95.5 5.81a3.02 3.02 0 002.12 2.14C4.47 20.5 12 20.5 12 20.5s7.53 0 9.38-.55a3.02 3.02 0 002.12-2.14C24 15.95 24 12 24 12s0-3.95-.5-5.81zM9.75 15.5v-7l6.5 3.5-6.5 3.5z"/></svg>
